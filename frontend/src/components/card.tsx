...
export const Card = ({
    ...
    return (
      <div
        className={`w-full ${isFull ? "md:w-[50%]" : "md:w-[14rem]"} bg-zinc-900 transition ease-in-out delay-150 hover:translate-x-6 hover:scale-100 duration-300 mb-4`}
      >
        <img
          src={image}
          alt={"A picture of " + title}
          className={`w-full ${isFull ? "md:w-full" : "md:w-[14rem]"} h-full md:[10rem] ${isFull ? "md:h-[20rem]" : "md:h-[14rem]"} object-cover`}
        />
        <div className={`p-2 bg-zinc-900 w-full ${isFull ? "md:w-full" : "md:w-[14rem]"} h-15rem] overflow-clip my-3`}>
          <div className="flex gap-4 items-start w-full">
            <img className="h-12 w-12 object-cover rounded-full" src={avatar} alt={"A picture of user"} />
            <div className="text-left">
              <p className="text-orange-500 font-light">{email}</p>
            </div>
          </div>
          <h2 className={`text-orange-500 font-bold my-2 text-xl ${!isFull && "truncate overflow-hidden ..."}`}>
            {title}
          </h2>
